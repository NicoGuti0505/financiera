...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    $identificacion = $_POST['identificacion'] ?? null; // Permite que factura sea null si no se proporciona
    $factura = $_POST['factura'] ?? null; // Permite que factura sea null si no se proporciona
    error_log("Identificación recibida: " . print_r($identificacion, true));
...
$params = [$identificacion];
    ...
    if($factura !== null){
        $sql .= ($identificacion !== null) ? " AND " : " WHERE ";
        $sql .= "inicio_factura = ?)";
        $params[] = $factura;
    }
