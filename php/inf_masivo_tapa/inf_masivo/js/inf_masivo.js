...
function cargarRegistros() {
    ...
    const regionf_id = document.getElementById('regionf_id').value; 
    const camposInfTapa = document.getElementById('camposInfTapa');
    
    // Limpiar registros existentes
    camposInfTapa.innerHTML = '';
    
    // Agregar div para el contador
    const contadorDiv = document.createElement('div');
    contadorDiv.id = 'totalRegistros';
    contadorDiv.classList.add('text-blue-500', 'text-center', 'mb-4');
    camposInfTapa.appendChild(contadorDiv);
    ...
    fetch(window.location.href, {
        ...
        body: `ajax=1&start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}&regionf_id=${encodeURIComponent(regionf_id)}`
    })
    .then(response => {
        if (!response.ok) {
    ...
    .catch(error => {
        loadingIndicator.remove();
        mostrarMensajeError('Error al cargar los registros: ' + error.message);
...
function agregarCampos(datos = null) {
    ...
    const primeraFila = crearPrimeraFila();
    const segundaFila = crearSegundaFila();
    const botonEliminar = crearBotonEliminar(contenedor);

    // Verificar que los elementos se crearon correctamente antes de agregarlos
    if (primeraFila instanceof HTMLElement) contenedor.appendChild(primeraFila);
    if (segundaFila instanceof HTMLElement) contenedor.appendChild(segundaFila);
    if (botonEliminar instanceof HTMLElement) contenedor.appendChild(botonEliminar);

    const camposInfTapa = document.getElementById('camposInfTapa');
    if (camposInfTapa instanceof HTMLElement) {
        camposInfTapa.appendChild(contenedor);
    } else {
        console.error("Error: 'camposInfTapa' no es un elemento HTML válido.");
    }
...
function crearPrimeraFila() {
    ...
    fila.innerHTML = `
        <div>
            ...
            <input type="text" name="nit[]" class="form-input" readonly required>
        </div>
        <div>
            <label class="form-label">Factura Inicial</label>
            <input type="text" name="fac_desde[]" class="form-input" readonly required>
        </div>
        <div>
...
function crearBotonEliminar(contenedor) {
    if (!(contenedor instanceof HTMLElement)) {
        console.error("Error: 'contenedor' no es un elemento HTML válido.");
        return document.createTextNode(""); // Devuelve un nodo vacío en lugar de `undefined`
    }

    const botonEliminar = document.createElement('button');
    botonEliminar.type = 'button';
    botonEliminar.classList.add(
        '-mt-20',  // Reduce el margen superior para subirlo
        'bg-red-500',
        'text-white',
        'px-3',
        'py-1',
    ...
    botonEliminar.addEventListener('click', function () {
        if (contenedor.parentNode) {
            ...
            console.warn("El contenedor ya no tiene un nodo padre.");
        }
    });

    const contenedorBoton = document.createElement('div');
    contenedorBoton.classList.add('text-center', 'mt-2');
...
function actualizarCampos(data, contenedor) {
    const campos = [
        'nit','fac_desde','fac_hasta', 'tercero_id', 'radicado', 'linea_rep', 'num_pedido',
...
function guardarDatos(validado = false) {
    ...
    const camposFaltantes = validarCamposObligatorios(datos);
    if (camposFaltantes.length > 0) {
        let mensaje = 'Faltan campos obligatorios:\n\n';
        camposFaltantes.forEach(({ registro, campos }) => {
    ...
    const confirmar = confirm('¿Está seguro de guardar los datos?');
    if (!confirmar) {
        return;
    }
    ...
    .then(response => response.json())
    .then(result => {
        procesarResultadoGuardado(result, validado);
        if (result.success) {
...
function validarCamposObligatorios(datos) {
    const camposRequeridos = [
        'nit', 'fac_desde', 'fac_hasta', 'tercero_id', 'linea_rep', 
        'num_pedido', 'linea_dis_ped', 'plan_contable', 
        'pago_causar_memorando', 'concepto', 'mes_servicio', 
        'contrato', 'crp_id'
    ];
    ...
    datos.forEach((registro, index) => {
        // Solo validar si tiene radicado
        if (registro.radicado && registro.radicado.trim() !== '') {
            const faltantes = camposRequeridos.filter(campo => 
                !registro[campo] || registro[campo].trim() === ''
            );
...
function procesarResultadoGuardado(result, validado) {
    if (result.success) {
        ...
        if (result.invalidCount > 0) {
            mensajeAdicional += `\n${result.invalidCount} registro(s) sin radicado ni voucher.`;
        }
        //if (result.noVoucherCount > 0) {
            //mensajeAdicional += `\n${result.noVoucherCount} registro(s) con radicado pero sin voucher.`;
        //  }
        if (mensajeAdicional) {
            alert('Atención:' + mensajeAdicional);
        }
    } else {
        let mensaje = result.message || 'Error desconocido al guardar los datos';
        if (result.invalidCount > 0) {
            mensaje += `\n${result.invalidCount} registro(s) sin radicado ni voucher.`;
        }
        //if (result.noVoucherCount > 0) {
