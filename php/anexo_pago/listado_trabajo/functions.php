...
function obtener_registros_por_fecha($conn, $startDate, $endDate) {
    try {
        if (empty($startDate) || empty($endDate)) {
            throw new Exception("Las fechas requeridas");
        }

        $registros = buscar_anexo($conn, null, null, $startDate, $endDate);
        if (empty($registros)) 
        {
            return []; // Retornar array vacío en lugar de lanzar excepción
        }

        return $registros;
    } catch (Exception $e) {
        error_log("Error en obtener_registros_por_fecha: " . $e->getMessage());
        throw $e;
    }
}

function buscar_anexo($conn, $startDate = null, $endDate = null, $regionf_id = null) {
    $grupor_id = (int)$_SESSION['grupor_id'];
    $sql = "SELECT DISTINCT
            vt.tercero_id AS id_tercero,
            vt.id AS validacion_terceros_id,
            ap.id AS anexo_pago_id,
            ap.iva,
            ap.base_iva,
            ap.base_excenta,
            ap.id_articulo AS articulo,
            t.identificacion AS nit,
            vt.valor_total AS valor_total,
            vt.fac_desde AS inicio_factura,
            vt.fac_hasta AS final_factura,
            t.municipio_id AS ubicacion,
            rf.descripcion AS descripcion
            FROM validacion_terceros vt
            LEFT JOIN anexo_pago ap ON vt.id = ap.validacion_terceros_id
            LEFT JOIN evento_tercero et ON vt.id = et.id_validacion_tercero
            JOIN tercero t ON vt.tercero_id = t.id
            LEFT JOIN region_fomag rf ON t.regionf_id = rf.id
            LEFT JOIN grupo_region gr ON rf.grupor_id = gr.id
            WHERE gr.id = ?";
    ...
    if ($startDate && $endDate) {
        ...
        $sql .= " AND et.[fecha] BETWEEN ? AND ?";
        $params[] = $startDateTime->format('Y-m-d H:i:s');
        $params[] = $endDateTime->format('Y-m-d H:i:s');

        
        $sql .= " AND (ap.id IS NULL OR ap.id_articulo = '' OR ap.id_articulo IS NULL)";
    ...
    try {
        ...
        foreach ($data as $row) {
            if (!$row['id_anexo']){
                ...
                $stmt = sqlsrv_query($conn, $sql, array($row['id_anexo']));
                if ($stmt === false) {
        ...
        if (!empty($existingRecords) && !$validado) {
            ...
            $result = enviardata($conn, $newRecords, $updatedRecords);
            return $result;
        }
    } catch (Exception $e) {
...
function enviardata($conn, $newRecords, $updatedRecords){
    try {
        ...
        return [
            ...
            'allRecords' => array_merge($newRecords, $updatedRecords) // Aseguramos que enviamos todos los registros
        ];
    } catch (Exception $e) {
...
function upData($data, $conn){
    ...
    foreach ($data as $row) {
        try {
            ...
            // Insertar en evento_anexo
            $insertSql = "INSERT INTO evento_anexo (anexo_pago_id, usuario_id, evento, fecha, descripcion) VALUES (?, ?, ?, ?, ?)";
            $insertParams = [$row['id_anexo'], $row['usuario_id'] , 'Actualización', $fecha_actual, $row['descripcion']];
            $insertStmt = sqlsrv_query($conn, $insertSql, $insertParams);
            ...
            sqlsrv_rollback($conn);
            error_log("Error en upData: " . $e->getMessage());
            // Puedes decidir si quieres lanzar la excepción o manejarla de otra manera
            throw $e;
        }
    }
}
